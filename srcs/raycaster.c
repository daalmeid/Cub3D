...
static void	ft_drawing(int line_height, char side, int x, t_player *p, int tex_x)
{
	...
	while (painter != draw_start)
		my_pixel_put(&p->map_data, map_width - x - 1, painter++, 0x4287f5);
	while (painter < draw_end)
	{
		...
		texPos += step;
		my_pixel_put(&p->map_data, map_width - x - 1, painter++, color);
	}	
	while (painter < map_height)
		my_pixel_put(&p->map_data, map_width - x - 1, painter++, 0x3fb821);
}
...
{
	rc->map_x = (int)p->pos.x;
	rc->map_y = (int)p->pos.y;
	if (rc->ray_dir_x < 0)
	{
		rc->step_x = -1;
		rc->side_dist_x = (p->pos.x - rc->map_x) * rc->delta_dist_x;
	}
	else
	{
		rc->step_x = 1;
		rc->side_dist_x = (rc->map_x + 1.0 - p->pos.x) * rc->delta_dist_x;
	}
	if (rc->ray_dir_y < 0)
	{
		rc->step_y = -1;
		rc->side_dist_y = (p->pos.y - rc->map_y) * rc->delta_dist_y;
	}
	else
	{
		rc->step_y = 1;
		rc->side_dist_y = (rc->map_y + 1.0 - p->pos.y) * rc->delta_dist_y;
	}
}
...
static void	delta_calc(int x, t_player *p, int world_map[24][24])
{
	...
	rc.ray_dir_x = p->dir.x + p->plane.x * camera_x;
	rc.ray_dir_y = p->dir.y + p->plane.y * camera_x;
	if (rc.ray_dir_x != 0)
	...
	if (side == 'W' || side == 'E')
		wall_x = p->pos.y + perp_wall_dist * rc.ray_dir_y;
	else
		wall_x = p->pos.x + perp_wall_dist * rc.ray_dir_x;
	wall_x -= floor((wall_x));
